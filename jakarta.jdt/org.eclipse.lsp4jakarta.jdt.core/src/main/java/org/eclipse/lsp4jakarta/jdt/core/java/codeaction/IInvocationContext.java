...
public interface IInvocationContext {
	 ...
	 * innermost of those nodes. Otherwise, returns the first node in a preorder
	 * traversal of the AST, where the complete node range is covered by the
	 * selection.
	 *
	 * @return the covered node, or <code>null</code> if the selection is empty or
	 *         too short to cover an entire node
	 */
	 ...
	 * covering node found in a preorder traversal of the AST. This implies that for
	 * a zero-length selection between two adjacent sibling nodes, the node on the
	 * right is returned.
	 * </p>
	 *
